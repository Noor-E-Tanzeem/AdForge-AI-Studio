...
def generate_with_llama(prompt):
    slogans = [
        "⚡ Power Your Day. Own the Moment.",
        "🔥 Feel the Boost. Live the Energy.",
        "🚀 Rise. Grind. Dominate.",
        "💥 Unleash Your Inner Power.",
        "✨ Energy That Moves You."
    ]
    scripts = [
        "Introducing the energy that keeps you unstoppable. Fuel your ambition and own every moment.",
        "From sunrise to midnight, stay energized and focused. One sip, infinite possibilities.",
        "Power up your dreams. Break limits. Stay unstoppable."
    ]
...
def generate_billboard(product, slogan, brand_color, cta, human_img=None, product_img=None):
    bg = Image.new("RGB", (1280, 720), (15, 18, 40))
    draw = ImageDraw.Draw(bg)

    try:
        font_title = ImageFont.truetype("DejaVuSans-Bold.ttf", 80)
        font_slogan = ImageFont.truetype("DejaVuSans.ttf", 44)
        font_cta = ImageFont.truetype("DejaVuSans-Bold.ttf", 48)
    except:
    ...
    draw.rectangle([0, 0, 1280, 120], fill=(50, 80, 140))

    draw.text((60, 30), f"✨ {product.upper()}", fill=(255,255,255), font=font_title)
    draw.text((60, 160), slogan, fill=(200,220,255), font=font_slogan)
    ...
    if product_img:
        prod = Image.open(product_img).convert("RGB").resize((300,300))
        bg.paste(prod, (60,360))

    # CTA button
    draw.rounded_rectangle([450, 580, 850, 670], radius=40, fill=(255, 80, 80))
    w, h = draw.textsize(cta, font=font_cta)
    draw.text((650 - w//2, 600), cta, fill=(255,255,255), font=font_cta)

    draw.text((60, 690), "🚀 AdForge AI Studio", fill=(140,140,180), font=font_slogan)
